...
create table if not exists public.assignments (
  ...
  valid_to date,
  notes text,
  assignment_type text not null default 'fixed' check (assignment_type in ('fixed', 'coverage', 'reinforcement', 'special')),
...
-- -----------------------------------------------------------------------------
-- Novedades justificadas y trazabilidad (2026-09-10)
-- -----------------------------------------------------------------------------
...
create index if not exists idx_attendance_leaves_operator_dates on public.attendance_leaves(operator_id, start_date, end_date);
create index if not exists idx_attendance_leaves_status on public.attendance_leaves(status);
create index if not exists idx_attendance_leave_audit_changed_at on public.attendance_leave_audit(changed_at desc);
...
create trigger trg_attendance_leaves_updated_at before update on public.attendance_leaves
...
begin
  ...
  if tg_op = 'INSERT' then
    insert into public.attendance_leave_audit(leave_id, action, new_data, changed_by) values (new.id, 'INSERT', to_jsonb(new), v_actor);
    return new;
  elsif tg_op = 'UPDATE' then
    insert into public.attendance_leave_audit(leave_id, action, old_data, new_data, changed_by) values (new.id, 'UPDATE', to_jsonb(old), to_jsonb(new), v_actor);
    return new;
  else
    insert into public.attendance_leave_audit(leave_id, action, old_data, changed_by) values (old.id, 'DELETE', to_jsonb(old), v_actor);
    return old;
  end if;
end;
$$;
...
create trigger trg_attendance_leaves_audit after insert or update or delete on public.attendance_leaves
...
create policy "attendance_leaves_select_relevant" on public.attendance_leaves for select to authenticated
using (public.is_supervisor() or operator_id = auth.uid());

drop policy if exists "attendance_leaves_write_supervisor" on public.attendance_leaves;
create policy "attendance_leaves_write_supervisor" on public.attendance_leaves for all to authenticated
...
create policy "attendance_leave_audit_select_supervisor" on public.attendance_leave_audit for select to authenticated
using (public.is_supervisor());

revoke all on public.attendance_leaves from anon;
revoke all on public.attendance_leave_audit from anon;
...
-- Jornada flexible excepcional (agregado 2026-09-10)
...
create index if not exists idx_flexible_attendance_modes_operator_dates on public.flexible_attendance_modes(operator_id, valid_from, valid_to);
create unique index if not exists idx_flexible_attendance_modes_one_open_per_operator on public.flexible_attendance_modes(operator_id) where valid_to is null;
drop trigger if exists trg_flexible_attendance_modes_updated_at on public.flexible_attendance_modes;
create trigger trg_flexible_attendance_modes_updated_at before update on public.flexible_attendance_modes for each row execute function public.set_updated_at();
...
create policy "flexible_attendance_modes_select_relevant" on public.flexible_attendance_modes for select to authenticated using (public.is_supervisor() or operator_id = auth.uid());
drop policy if exists "flexible_attendance_modes_write_supervisor" on public.flexible_attendance_modes;
create policy "flexible_attendance_modes_write_supervisor" on public.flexible_attendance_modes for all to authenticated using (public.is_supervisor()) with check (public.is_supervisor());
